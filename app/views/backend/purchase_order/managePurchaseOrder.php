<?php 
	$purchase_order = $purchaseOrderMenu = accessMenu(purchase_order);
?>
						...
						<div class="header-lines">
							...
							<div class="row mb-3">
								...
								<div class="col-md-6 text-right">
									...
									?>
									<a href="<?php echo base_url(); ?>purchase_order/managePurchaseOrder" class="btn btn-default btn-sm">Close</a>
								</div>
							</div>
							<!-- Buttons end here -->
							...
							<fieldset <?php echo $fieldSetDisabled;?>>
								...
								<section class="header-section">
										...
										<div class="col-md-4">
											<div class="row">
												...
												<div class="form-group col-md-6">
													<input type="text" name="po_number" id="po_number" readonly autocomplete="off" class="form-control no-outline" value="<?php echo isset($edit_data[0]['po_number']) ? $edit_data[0]['po_number'] : NULL;?>" placeholder="">
												</div>
											</div>
										</div>
													...
													<script>
														...
														function selectSupplier(val)
														{
															if(val !='')
															{
																$.ajax({
																	type: "POST",
																	url:"<?php echo base_url().'purchase_order/getAjaxSupplierDetails';?>",
																	data: { id: val }
																}).done(function( msg ) 
																{   
														...
														function getAjaxSupplierSiteDetails(val)
														{
															if(val !='')
															{
																...
																$.ajax({
																	type: "POST",
																	url:"<?php echo base_url().'purchase_order/getAjaxSupplierSiteDetails';?>",
									...
									<div class="row">
										<div class="col-md-4">
											<div class="row">
												<label class="col-form-label col-md-6 text-right"><span class="text-danger">*</span> PO Date</label>
												<div class="form-group col-md-6">
													<input type="text" name="po_date" id="po_date" readonly autocomplete="off" required class="form-control no-outline -default_date" value="<?php echo isset($edit_data[0]['po_date']) ? date("d-M-Y",strtotime($edit_data[0]['po_date'])) : date("d-M-Y");?>" placeholder="">
												</div>
											</div>
										</div>
										...
										<table class="table table-bordered table-hover line_items" id="line_items">
											<thead>
												<tr>
													...
													<th class="tab-md-100"><span class="text-danger">*</span> Base Price</th>
													<th class="tab-md-100">Tax</th>
													<th class="tab-md-100">Discount Type</th>
													<th class="tab-md-100">Discount</th>
													<th class="tab-md-150">Discount Reason</th>
													<th class="tab-md-100">Price</th>
													<th class="tab-md-100">Line Value</th>
					...
					<script>
							...
							$.ajax({
								...
								success: function(d)
								{
									...
									$("table.line_items").find('input[name^="text_product_id[]"]').each(function () 
									{
										...
										if(text_product_id == 0 || quantity == "" || base_price == "" || delivery_date== "") //|| uom == ""
										{
											flag = 1;
										}
									});
						...
						function selectItemDetails(item_id,counter)
						{
							if(item_id)
							{
								...
								$.ajax({
									type: "POST",
									url:"<?php echo base_url().'purchase_order/getLineItems';?>",
						...
						function selectItemUom(item_id,counter)
						{
							if(item_id)
							{
								$.ajax({
									type: "POST",
									url:"<?php echo base_url().'purchase_order/ajaxSelectItemUom';?>",
									data: { id: item_id }
								}).done(function( result ) 
								{   
				...
				{ 
					...
					<div class="row">
						...
						<div class="col-md-6 float-right text-right">
							<?php
								if($purchase_order['create_edit_only'] == 1 || $this->user_id == 1)
								{
									?>
									<a href="<?php echo base_url(); ?>purchase_order/managePurchaseOrder/add" class="btn btn-info btn-sm">
					...
					<form action="" class="form-validate-jquery" method="get">
						...
						<div class="row">
							...
							<div class="col-md-2">
								<button type="submit" --id="submit_btn" class="btn btn-info">Search <i class="fa fa-search" aria-hidden="true"></i></button>
								<a href="<?php echo base_url(); ?>purchase_order/managePurchaseOrder" title="Clear" class="btn btn-default">Clear</a>
							</div>
						</div>
						<!-- <script>
								...
								<table id="myTable" class="table table-bordered table-hover">
									<thead>
										<tr>
											...
											<th class="tab-md-100 text-center">PO Status</th>
											<th class="tab-md-120 text-left">Organization</th>
											<th class="tab-md-150 text-left">Branch</th>
											<th class="tab-md-80 text-left">PO #</th>
											<th class="tab-md-120 text-left">PO Description</th>
											<th class="tab-md-120 text-left">Status</th>
											<th class="tab-md-120 text-left">Purchase Date</th>
											<th class="tab-md-120 text-left">Supplier</th>
											<th class="tab-md-120 text-left">Supplier Site</th>
											<th class="tab-md-120 text-left">Currency</th>
												...
												<tr>
														...
														<div class="dropdown" style="width:90px;">
															...
															<ul class="dropdown-menu dropdown-menu-right dropdown-menu-new">
																<?php
																	if($purchase_order['create_edit_only'] == 1 || $this->user_id == 1)
																	{
																		if($row['po_status'] == "Pending Approval" || $row['po_status'] == "Info Requested")  #Pending Approval
																		{
																			?>
																			<li>
																				<a onclick="return confirm('Approval will be withdrawn need to submit for approval again, do you want to continue?')" href="<?php echo base_url(); ?>purchase_order/managePurchaseOrder/edit/<?php echo $row['po_header_id'];?>/pending_approval">
																		...
																		else if($row['po_status'] == "Approved")
																		{
																			?>
																			<li>
																				<a onclick="return confirm('PO will go for re-approval, do you want to continue?')" href="<?php echo base_url(); ?>purchase_order/managePurchaseOrder/edit/<?php echo $row['po_header_id'];?>/re_request">
																					<i class="fa fa-pencil"></i> Edit
																				</a>
																			</li>
																			<?php
																		}
																		else if($row['po_status'] == "Draft" || $row['po_status'] == "Withdrawn" || $row['po_status'] == "Rejected")
																		{
																			?>
																			<li>
																				<a href="<?php echo base_url(); ?>purchase_order/managePurchaseOrder/edit/<?php echo $row['po_header_id'];?>/re_request">
																					<i class="fa fa-pencil"></i> Edit
																				</a>
																			</li>
																			<?php
																	...
																	if($purchase_order['read_only'] == 1 || $this->user_id == 1)
																	{
																		?>
																		<li>
																			<a href="<?php echo base_url(); ?>purchase_order/managePurchaseOrder/view/<?php echo $row['po_header_id'];?>">
																	...
																	if($purchase_order['create_edit_only'] == 1 || $this->user_id == 1)
																	{
																		?>
																		<li>
																			<a target="_blank" href="<?php echo base_url(); ?>purchase_order/viewApprovals/<?php echo $row['po_header_id'];?>">
																...
																<li>
																	<a target="_blank" href="<?php echo base_url(); ?>purchase_order/generatePDF/<?php echo $row['po_header_id'];?>">
													...
													<td  class='text-center'>
														<?php
															if($purchase_order['create_edit_only'] == 1 || $this->user_id == 1)
															{
																if($row['po_status'] != "Closed" && $row['po_status'] != "Cancelled")
																{
																	?>
																	<div class="dropdown">
																		<button type="button" class="btn btn-outline-info gropdown-toggle btn-sm" data-toggle="dropdown" aria-expanded="false">
																		...
																		<ul class="dropdown-menu dropdown-menu-right">
																			<li>
																				<a href="<?php echo base_url(); ?>purchase_order/managePurchaseOrder/approval_status/<?php echo $row['po_header_id'];?>/Closed" onclick="return confirm('Are you sure update close status?')">
																			...
																			<li>
																				<a href="<?php echo base_url(); ?>purchase_order/managePurchaseOrder/approval_status/<?php echo $row['po_header_id'];?>/Cancelled" onclick="return confirm('Are you sure update cancel status?')">
													...
													<td><?php echo $row['po_number'];?> <?php #echo $row['po_status']; ?></td>
