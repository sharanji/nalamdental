...
<script>
	...
	JSPM.JSPrintManager.license_url = "<?php echo base_url();?>"; // LICENSE PORT
	JSPM.JSPrintManager.auto_reconnect = true;
	JSPM.JSPrintManager.start();
			...
			<div class="search-form" <?php echo $displaySearch;?>>
				<form action="" class="form-validate-jquery" method="get" >
					<div class="row mt-3">
						<div class="col-md-4">
							<div class="row">
								<label class="col-form-label col-md-4">Order Number</label>
								<div class="form-group col-md-7">
									<input type="search" name="order_number" id="order_number" value="<?php echo isset($_GET['order_number']) ? $_GET['order_number'] : ""; ?>" class="form-control">
								</div>
							</div>
						</div>
						<div class="col-md-4">
						...
						<div class="col-md-4">
							<div class="row">
								...
								<div class="form-group col-md-8">
									...
									?>
									<select style="width:228px;" name="payment_type_id" id="payment_type_id" class="form-control searchDropdown">
...
<script>
                    ...
                    if( countKey > 0 )
                    {
                        $.each(print_items, function(i, item) 
                        {
                            ...
                            if(print_type == "KOT")
                            {
                                ...
                                for(i=1; i<=printer_count; i++) //printer_count
								{
									//kotAutoPrint(jspmWSStatus(),htmlKOTContent,orderID,printer_name);
									kotAutoPrint(htmlKOTContent,orderID,printer_name);
								}	
                            }
                        });
                    }  
                        ...
                        $.each(print_items, function(i, item) 
                        {
                            ...
                            if(print_type == "CASHIER")
                            {
								...
								for(i=1; i<=printer_count; i++)
								{
									...
									orderAutoPrint(htmlCashierContent,orderID,printer_name);
								}
                            }
                        });
                    }
    ...
    function kotAutoPrint(htmlContent,orderID,printer_name)
    {
	...
	function orderAutoPrint(htmlContent,orderID,printer_name)
    {
        /* if (printerStatus && htmlContent !="") 
        {
            //Create a ClientPrintJob
    ...
    function updateOnlineOrderStatus(orderID)
    {
		if(orderID)
        {
            ...
            $.ajax({
                url      : '<?php echo base_url(); ?>billGenrator/updateOnlineOrderStatus/'+orderID,
                type     : "POST",
                data     : {},
                datatype : JSON,
                success  : function(result)
